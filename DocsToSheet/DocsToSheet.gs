function extractFilteredLinksFromDocsViaURL() {
  ...
  if (!fileIdMatch) {
    Logger.log('❌ Invalid Google Drive link format. Please use a full public link like https://drive.google.com/file/d/...');
    return;
  }
  ...
  try {
    ...
    Logger.log(`❌ Unable to access file. Make sure it's publicly shared. (${e.message})`);
    return;
  }
  ...
  try {
    ...
    Logger.log(`❌ Failed to read file content. It may not be a valid .txt file. (${e.message})`);
    return;
  }
  ...
  for (let i = 0; i < totalDocs; i++) {
    ...
    try {
      ...
      Logger.log(`📄 Processing ${i + 1}/${totalDocs}: ${docName}`);
  ...
  Logger.log(`⏱️ Total time: ${elapsedMin}m ${elapsedSec}s`);
}
