...
def process_link(driver, title, link, idx):
    ...
    if REMOVE_EXTENSIONS:
        ...
        for f in removed:
            print('   [REMOVE_EXTENSIONS] Removed from title folder:', f)
    ...
    for title, links in grouped.items():
        for idx, link in enumerate(links, start=1):
            ...
            if res['status'] == 'done' and res.get('files'):
                overall['success'] += 1
                print('   [Success] Downloaded:', res.get('files'))
            ...
            print(f'   [Time] Avg {avg:.1f}s/link — est remaining {est//60}m {est%60}s')
        ...
        if last_per_link_tmp and last_title_dir and os.path.isdir(last_per_link_tmp):
            ...
            if REMOVE_EXTENSIONS:
                ...
                for f in removed_after_wait:
                    print('   [Removed] Removed from title folder after final wait:', f)
